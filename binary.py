...
def generate(width, height, stripe_type=0, inverse=False):
    ...
    #generate
    binary_pattern = np.flip(np.fromfunction(lambda y,x,d: (x&(1<<d)!=0), (height,width,depth), dtype=int).astype(np.uint8), axis=2)
    
    if stripe_type!=0:
        binary_pattern = binary_pattern.transpose(1, 0, 2)

    if inverse==True:
        binary_pattern = True - binary_pattern
    
    binary_pattern *= 255
    imgs_binary = list(binary_pattern.transpose(2, 0, 1))
    return imgs_binary
...
def decode(imgs_posi, imgs_nega):
    ...
    binary_pattern = np.empty((height, width, depth), dtype=np.uint8)
    binary_pattern[posi>=nega] = True
    binary_pattern[posi<nega] = False
...
def main():
    ...
    #decode binary code images
    img_decode = decode(imgs_posi, imgs_nega)

    #export binary code images
    out_dir = args.out
    ...
    print("Export binary code images")
